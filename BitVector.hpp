...
#include "sim_ds/BitReference.hpp"
...
class _BitVector {
   ...
   protected:
    ...
    explicit _BitVector(size_t size)
        : actual_size_(size), storage_(get_word_size(size)) {}

    explicit _BitVector(size_t size, bool initial_bit)
        : actual_size_(size), storage_(get_word_size(size)) {
        _fill(_make_iter(0), _make_iter(size), initial_bit);
    }

    size_t get_word_size(size_t bit_size) const {
        return (bit_size == 0 ? 1 : (bit_size - 1) / kBitsPerWord + 1);
    }

    _reference _make_ref(size_t pos) {
        return _reference(storage_.data() + pos / kBitsPerWord,
                          bit_util::OffsetMask(pos % kBitsPerWord));
    }

    _const_reference _make_ref(size_t pos) const {
        return _const_reference(storage_.data() + pos / kBitsPerWord,
                                bit_util::OffsetMask(pos % kBitsPerWord));
    }

    _iterator _make_iter(size_t pos) {
        return _iterator(storage_.data() + pos / kBitsPerWord,
                         pos % kBitsPerWord);
    }

    _const_iterator _make_iter(size_t pos) const {
        return _const_iterator(storage_.data() + pos / kBitsPerWord,
                               pos % kBitsPerWord);
    }
    ...
    void _fill(_iterator begin, _iterator end, bool bit) {
        ...
        if (bit) {  // Fill by 1
            if (begin.seg_ < back.seg_) {
                ...
                *back.seg_ |=
                    end.ctz_ > 0 ? compl(mask_fill << end.ctz_) : mask_fill;
            } else {
                *begin.seg_ |=
                    ((mask_fill << begin.ctz_) bitand (mask_fill >> eclz));
            }
        } else {  // Fill by 0
            if (begin.seg_ < back.seg_) {
                ...
                *back.seg_ &=
                    end.ctz_ > 0 ? mask_fill << end.ctz_ : _word_type(0);
            } else {
                *begin.seg_ &=
                    compl((mask_fill << begin.ctz_) bitand (mask_fill >> eclz));
            }
        }
    }
...
class BitVector : private _BitVector {
   ...
   public:
    ...
    void push_back(bool bit) { resize(size() + 1, bit); }

    void fill(iterator begin, iterator end, bool bit) {
        _fill(begin, end, bit);
    }

    void resize(size_t new_size) { _resize(new_size, 0); }

    void resize(size_t new_size, bool bit) { _resize(new_size, bit); }

    void assign(size_t new_size, bool bit) { _assign(new_size, bit); }

    void reserve(size_t reserved_size) { _reserve(reserved_size); }

    void shrink_to_fit() { _base::storage_.shrink_to_fit(); }
    ...
    // Pointer Must be aligned as 32 bytes.
    // こっち使う
    _const_word_pointer data() const {
        // std::cout << _base::storage_.data()<<std::endl;
        return _base::storage_.data();
    }
