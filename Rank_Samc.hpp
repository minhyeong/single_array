...
class string_array_explorer {
   public:
    ...
    static_assert(std::is_convertible_v<typename iterator_traits::value_type,
                                        std::string>);
   ...
   public:
    ...
    explicit string_array_explorer(Iter begin, Iter end, size_t depth)
    ...
    void for_each_edge(Action action) const {
        ...
        while (it != end_) {
            auto c = (*it)[depth_];
            auto endit = it;
            while (endit != end_ and (*endit)[depth_] == c) ++endit;
...
class _SamcImpl {
   ...
   protected:
    ...
    explicit _SamcImpl(StrIter begin, StrIter end)
        : _SamcImpl(string_array_explorer(begin, end, 0)) {}

    template <typename T, typename S>
    [[deprecated]] explicit _SamcImpl(const graph_util::Trie<T, S>& trie);

   private:
    using mask_type = uint64_t;
    static constexpr size_t kMaskWidth = 64;

    position_type y_check_(const std::vector<size_t>& indices,
                           const BitVector& empties) const;

    position_type y_check_legacy_(const std::vector<size_t>& indices,
                                  const BitVector& empties) const;

    size_t shifts_of_conflicts_(mask_type fields, mask_type mask,
                                mask_type conflicts) const;
};
...
_SamcImpl<CodeType>::_SamcImpl(const string_array_explorer<Iter>& explorer) {
    ...
    for (size_t depth = 0;; ++depth) {
        ...
        std::cerr << "depth: " << depth
                  << ", block_height: " << head_[depth + 1] - head_[depth]
                  << std::endl;
#endif
        std::array<std::vector<size_t>, kAlphabetSize> indices_table;
        std::array<std::vector<string_array_explorer<Iter>>, kAlphabetSize>
            node_table;
        while (not node_queue.empty()) {
            auto node_pair = node_queue.front();
            node_queue.pop();
        ...
        for (size_t c : code_out) {
            ...
            if (c == kLeafChar) continue; // 終端文字除去
#ifndef NDEBUG
            std::cerr << c << ':' << uint8_t(c)
                      << ", indices: " << indices.size() << std::endl;
#endif
            auto& nodes = node_table[c];
            auto indices_height = 1 +
        ...
        std::cerr << "used: " << std::fixed << std::setprecision(2) << " %"
                  << per_used << std::endl
                  << std::endl;
#endif
    }
...
_SamcImpl<CodeType>::_SamcImpl(const graph_util::Trie<T, S>& trie) {
    ...
    while (max_index >= 0) {
        ...
        for (size_t i = 0; i < height; i++) {
            ...
            if (storage_[index] == kEmptyChar) continue;
        ...
        for (size_t i = 0; i < kAlphabetSize; i++) {
            ...
            max_index =
                std::max(max_index, y_front + position_type(indices.back()));
            ...
            for (auto id : indices) {
                ...
                assert(storage_[abs_index] == kEmptyChar and empties[index]);
...
template <typename CodeType>
typename _SamcImpl<CodeType>::position_type _SamcImpl<CodeType>::y_check_(
    const std::vector<size_t>& indices, const BitVector& empties) const {
    const auto word_size = (empties.size() - 1) / kMaskWidth + 1;
    auto field_bits = [&](size_t i) {
        return i < word_size ? empties.data()[i] : 0;
    };
    assert(position_type(empties.size()) + indices.front() - indices.back() >=
           0);
    auto field_size =
        (empties.size() + indices.front() - indices.back()) / kMaskWidth + 1;
    ...
    for (size_t i = 0; i < field_size; i++) {
        ...
        for (auto id : indices) {
            ...
            if (insets == 0) {
                ...
                candidates &= (field_bits(p + i) >> insets) |
                              (field_bits(p + i + 1) << (kMaskWidth - insets));
            }
            if (candidates == 0) break;
        }
        if (candidates)
            return position_type(kMaskWidth) * i + bit_util::ctz(candidates) -
                   heads;
    }
...
_SamcImpl<CodeType>::y_check_legacy_(const std::vector<size_t>& indices,
                                     const BitVector& empties) const {
    ...
    auto field_bits_at = [&](long long index) -> mask_type {
        return index < (empties.size() - 1) / kMaskWidth + 1
                   ? ~empties.data()[index]
                   : 0;
    };
    ...
    auto check = [&](position_type offset) -> size_t {
        ...
        if (index_end - index_front ==
            n_empties)  // All of values are corresponding to expanded area.
            return 0;
        if (n_empties <
            indices.size()) {  // There are not enough number of empty areas.
            ...
            if (not empties[offset + id]) {
                ...
                if (p < 0) {
                    ...
                    if (insets == 0)
                        ...
                        field =
                            (field_bits_at(pi) >> insets) |
                            (field_bits_at(pi + 1) << (kMaskWidth - insets));
                }
                size_t shifts = 0;
                mask_type conflicts = mask & field;
                while ((shifts += shifts_of_conflicts_(
                            field, mask << shifts, conflicts)) < kMaskWidth and
                       (conflicts = (mask << shifts) & field))
                    continue;
                return shifts;
            }
        }
...
template <typename CodeType>
size_t _SamcImpl<CodeType>::shifts_of_conflicts_(mask_type fields,
                                                 mask_type mask,
                                                 mask_type conflicts) const {
    ...
    auto mask_followings =
        clz(~(mask << (kMaskWidth - 1 - conflict_front))) - 1;
...
class Samc : _SamcImpl<CodeType> {
   ...
   public:
    ...
    void Write(std::ostream& os) const { _base::Write(os); }
...
Samc<CodeType>::accept(std::string_view key) const {
    ...
    for (; depth < key.size(); depth++) {
        ...
        if (not in_range(target, depth + 1) or _base::check(target) != c) {
            //------
            c |= (1 << 7);
            if(key.size()-1==depth and  // 終端の場合
            (not in_range(target, depth + 1) or _base::check(target) != c) ){
                return true;
            }
            //------
            return false;
    ...
    /*int msb_1=0, cc=0;
    for(size_t v : storage_) {
        //v |= (1 << 7);
        if(std::bitset<8>(v)[7]==0){
            msb_1++;
        }
        cc++;
        std::cout << "bit : " << (std::bitset<8>(v)) << std::endl;
    }
    std::cout << msb_1 << " / " << cc <<std::endl;*/
...
class _SamcDictImpl : protected _SamcImpl<CodeType> {
   ...
   public:
    _SamcDictImpl() : _base() { _setup_leaves(); }
    ...
    void _setup_leaves() {
        ...
        for (size_t i = 1; i < _base::storage_.size(); i++) {
            if (_base::check(i) == kLeafChar) leaves_src[i] = true;
        }
    ...
    size_t size_in_bytes() const {
        return _base::size_in_bytes() + leaves_.size_in_bytes();
    }
...
template <typename CodeType>
size_t SamcDict<CodeType>::lookup(std::string_view key) const {
    size_t node = 0;
    size_t depth = 0;
    for (; depth < key.size(); depth++) {
        uint8_t c = key[depth];
...
template <typename CodeType>
std::string SamcDict<CodeType>::access(size_t value) const {
    std::string text;
    size_t node = _base::leaf(value);
    size_t depth =
        std::upper_bound(_base::head_.begin(), _base::head_.end(), node) -
        _base::head_.begin() - 1;
    while (depth > 0) {
